...
function drawBoundary() {
    for (let i = 1; i <= 116; i++) {
        for (let j = 1; j <= 116; j++) {
            ...
            if (getElementByCoord(i, j - 1) &&
                getElementByCoord(i, j - 1).classList.contains("border-all-missing")) {
...
function drawFixedBuilding(type) {
    coord_building_stone[type - 3].map(function(v) {
        let unit = v.split("-")
        insertBuilding(Number(unit[0]), Number(unit[1]), Number(unit[2]), "false", text_stone, "#000000", color_stone, "#000000")
    })
    coord_building_copper[type - 3].map(function(v) {
        let unit = v.split("-")
        insertBuilding(Number(unit[0]), Number(unit[1]), Number(unit[2]), "false", text_copper, "#000000", color_copper, "#000000")
    })
    coord_building_wood[type - 3].map(function(v) {
        let unit = v.split("-")
        insertBuilding(Number(unit[0]), Number(unit[1]), Number(unit[2]), "false", text_wood, "#000000", color_wood, "#000000")
    })
    coord_building_clay[type - 3].map(function(v) {
        let unit = v.split("-")
        insertBuilding(Number(unit[0]), Number(unit[1]), Number(unit[2]), "false", text_clay, "#000000", color_clay, "#000000")
    })
    coord_building_wharf[type - 3].map(function(v) {
        let unit = v.split("-")
        insertBuilding(Number(unit[0]), Number(unit[1]), Number(unit[2]), "false", text_wharf, "#000000", color_wharf, "#000000")
    })
}
